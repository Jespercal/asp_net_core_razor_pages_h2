...
{
    public class FavoritesDeleteModel : PageModel
    {
        ...
        public FavoritesDeleteModel(ILogger<FilledPageModel> logger, IMyRepository repository, UserManager<IdentityUser> userManager )
        {
            _logger = logger;
        ...
        public async Task<IActionResult> OnPost( int contactId )
        {
            if(ModelState.IsValid)
            {
                ...
                if(user != null)
                {
                    _repository.DeleteFavorite(user, contactId);
